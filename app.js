...
function typeEmoji(type) {
  const map = {
    normal: '⭐', fire: '🔥', water: '💧', grass: '🍃', electric: '⚡', ice: '❄️', fighting: '🥊', poison: '☠️', ground: '🌋',
    flying: '🪽', psychic: '🔮', bug: '🐛', rock: '🪨', ghost: '👻', dragon: '🐉', dark: '🌚', steel: '⚙️', fairy: '✨'
  };
  return map[type] || '🎲';
}
...
function randomIntro(name, types, height, weight) {
  ...
  let b;
  do { b = lines[randInt(0, lines.length - 1)]; } while (b === a);
  return `${a} ${b}`;
}
